...
| `SEED_ON_DEPLOY` | No | **Test/demo environments only.** Makes `npm run seed:deploy` (run by `deploy.sh` and Render's `preDeployCommand`) seed the treaty test portfolio on redeploy: `1` seeds only when the database has no seeded treaties yet; `reset` wipes the previous seed and reseeds every deploy. Unset (the default) it does nothing, so production deploys stay clean. See `docs/seed-test-treaties.md`. |
