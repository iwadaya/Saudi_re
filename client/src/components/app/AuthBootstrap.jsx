...
function BootOffline({ onRetry, reason }) {
  // A 429 is the server explicitly saying "slow down" — telling the user to
  // check their connection would send them debugging the wrong thing.
  const rateLimited = reason === 'rate-limited';
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      <div style={{ maxWidth: 360, textAlign: 'center', color: '#d7dceb' }}>
        <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 8 }}>
          {rateLimited ? 'The server is busy' : 'Can’t reach the server'}
        </div>
        <p style={{ fontSize: 13, lineHeight: 1.5, color: 'rgba(215,220,235,0.7)', margin: '0 0 16px' }}>
          {rateLimited
            ? 'Too many requests right now — you’re still signed in. Wait a few seconds and try again.'
            : 'We couldn’t verify your session. You’re still signed in — check your connection and try again.'}
        </p>
...
export default function AuthBootstrap({ children }) {
  ...
  // Why the last verify failed: 'network' | 'rate-limited'. Drives the
  // BootOffline copy so a 429 isn't misreported as a connectivity problem.
  ...
  const verify = useCallback(async () => {
    ...
    try {
      ...
      if (err?.status === 401) {
        ...
        // Network error / timeout / 5xx / 429 → keep the session, offer a retry.
        setFailReason(err?.status === 429 ? 'rate-limited' : 'network');
        setPhase('offline');
      }
    }
  }, []);
  ...
  if (phase === 'offline') return <BootOffline onRetry={verify} reason={failReason} />;
  return children;
}
