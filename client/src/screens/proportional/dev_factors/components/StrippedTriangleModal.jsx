...
export default function StrippedTriangleModal({ onClose, stripLargeCat, exclusions, fullMatrix, strippedMatrix, years, numDevYears }) {
  return (
    ...
    >
      <div role="dialog" aria-modal="true" className="glass" style={{ width: 'min(1100px,97vw)', maxHeight: '88vh', overflow: 'auto', borderRadius: 16, border: '1px solid var(--stroke-soft)', background: 'var(--panel-bg-strong)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid var(--stroke-soft)' }}>
          ...
          <button onClick={onClose} style={{ width: 30, height: 30, borderRadius: 8, border: '1px solid var(--stroke-soft)', background: 'transparent', color: 'rgba(var(--text-rgb),.7)', cursor: 'pointer', fontSize: 14 }}>✕</button>
        </div>
        <div style={{ padding: 20 }}>
          <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: '0.04em', color: 'var(--accent-amber)', marginBottom: 8 }}>FULL (ORIGINAL — BEFORE STRIPPING)</div>
          <TriangleGrid matrix={fullMatrix} years={years} numDevYears={numDevYears} />
          <div style={{ fontSize: 12, fontWeight: 800, letterSpacing: '0.04em', color: 'var(--accent)', margin: '20px 0 8px' }}>{stripLargeCat ? 'STRIPPED (ATTRITIONAL)' : 'ATTRITIONAL BASIS (reference — not active)'}</div>
