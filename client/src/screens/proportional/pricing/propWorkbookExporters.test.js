...
const mocks = vi.hoisted(() => ({
  buildContractWorkbook: vi.fn(async () => ({ sheets: 1 })),
  fetchJson: vi.fn(async () => null),
}));

vi.mock('../../../utils/contractWorkbook', () => ({
  buildContractWorkbook: mocks.buildContractWorkbook,
...
describe('propWorkbookExporters quote routing', () => {
  beforeEach(() => {
    mocks.buildContractWorkbook.mockClear();
  ...
  it('uses quote endpoints when quote mode is enabled', async () => {
    ...
    expect(mocks.fetchJson).toHaveBeenNthCalledWith(1, '/api/quotes/quote-123', undefined);
    expect(mocks.fetchJson).toHaveBeenNthCalledWith(2, '/api/quotes/quote-123/documents', undefined);
    expect(mocks.fetchJson).toHaveBeenNthCalledWith(3, '/api/quotes/quote-123/triangles/PREMIUM', undefined);
    expect(mocks.fetchJson).toHaveBeenNthCalledWith(4, '/api/quotes/quote-123/pricing-yearly', undefined);
    expect(mocks.fetchJson).toHaveBeenNthCalledWith(5, '/api/quotes/quote-123/pricing-outputs', undefined);
    expect(mocks.fetchJson).toHaveBeenNthCalledWith(6, '/api/quotes/quote-123/cresta', undefined);
  });
  ...
  it('keeps treaty endpoints for treaty mode', async () => {
    ...
    expect(mocks.fetchJson).toHaveBeenNthCalledWith(1, '/api/treaties/contract-456/documents', undefined);
    expect(mocks.fetchJson).toHaveBeenNthCalledWith(2, '/api/treaties/contract-456/loss-selection/CAT/latest', undefined);
  });
  ...
  it('forwards quote context into buildContractWorkbook', async () => {
    ...
    expect(mocks.buildContractWorkbook).toHaveBeenCalledTimes(1);
    const args = mocks.buildContractWorkbook.mock.calls[0][0];
