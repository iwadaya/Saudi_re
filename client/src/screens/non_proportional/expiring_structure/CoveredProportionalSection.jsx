  ...
  return (
    <section className="np-struct-card glass">
        ...
        <div style={{ padding: '16px 18px', borderBottom: '1px solid var(--hairline)', background: 'rgba(0,212,255,0.03)' }}>
          ...
          <div style={{ display: 'flex', gap: 16, alignItems: 'center', marginBottom: 16, flexWrap: 'wrap' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '0 0 auto' }}>
              <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.08em', color: 'rgba(var(--text-rgb),0.58)', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>Country</span>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flex: '0 0 auto' }}>
              <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: '.08em', color: 'rgba(var(--text-rgb),0.58)', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>Cedant</span>
            ...
            {!dbLoading && (
              <span style={{ fontSize: 11, color: 'rgba(var(--text-rgb),0.58)', marginLeft: 'auto' }}>
          ...
          {!dbLoading && dbProgrammes.length === 0 ? (
            <div style={{ fontSize: 12, color: 'rgba(var(--text-rgb),0.58)', padding: '10px 0', textAlign: 'center' }}>
                ...
                <thead>
                  <tr style={{ background: 'var(--surface-hover)' }}>
                    {['UW Year','Cedant','Treaty Type','Country','Status',''].map(h => (
                      <th key={h} style={{ padding: '7px 12px', textAlign: 'left', fontSize: 10, fontWeight: 700, letterSpacing: '.08em', color: 'rgba(var(--text-rgb),0.7)', textTransform: 'uppercase', borderBottom: '1px solid var(--hairline)' }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {dbProgrammes.map((p, i) => (
                    <tr key={i}
                      ...
                      <td style={{ padding: '7px 12px', color: 'rgba(var(--text-rgb),0.7)', fontVariantNumeric: 'tabular-nums' }}>{p.uw_year || '—'}</td>
                      <td style={{ padding: '7px 12px', color: 'var(--text)', fontWeight: 600 }}>{p.cedant_name || '—'}</td>
                      <td style={{ padding: '7px 12px', color: 'rgba(var(--text-rgb),0.78)' }}>{p.treaty_type_name || '—'}</td>
                      <td style={{ padding: '7px 12px', color: 'rgba(var(--text-rgb),0.58)', fontSize: 11 }}>{p.country_name || '—'}</td>
                      <td style={{ padding: '7px 12px' }}>
                        <span style={{ fontSize: 9, padding: '2px 7px', borderRadius: 6, background: 'var(--surface-hover)', color: 'rgba(var(--text-rgb),0.58)', fontWeight: 700, letterSpacing: '.06em' }}>
                  ...
                  <div className="np-cell-input">
                    {mode === 'db' && r.contractId ? (
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:6, width:'100%' }}>
                        ...
                        <button type="button"
                          style={{ background:'none', border:'none', color:'rgba(var(--text-rgb),0.58)', cursor:'pointer', fontSize:13, padding:'0 2px', flexShrink:0 }}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', borderTop: '1px solid var(--hairline)' }}>
        ...
        {rows.length > 0 && (
          <div style={{ fontSize: 11, color: 'rgba(var(--text-rgb),0.58)' }}>
