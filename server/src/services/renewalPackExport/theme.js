...
export function coverSheet(wb, tabs, meta) {
  ...
  t.value = { richText: [
    ...
    { text: 'Portfolio Renewal Pack', font: { name: UT.font, size: 15, color: { argb: 'FFEAF4F8' } } },
  ]};
  ...
  const meta1 = ws.getCell(6, 2); meta1.value = 'Generated';
  ws.getCell(6, 3).value = (meta?.date || new Date().toISOString().slice(0,10));
  const meta2 = ws.getCell(7, 2); meta2.value = 'Base currency';
  ws.getCell(7, 3).value = 'USD (converted at latest reference rates)';
  const meta3 = ws.getCell(8, 2); meta3.value = 'Basis';
  ws.getCell(8, 3).value = 'Triangles: all contracts · Profiles & aggregates: signed contracts';
  [meta1, meta2, meta3].forEach(c => c.font = { name: UT.font, bold: true, color: { argb: UT.teal } });
  [6,7,8].forEach(r => ws.getCell(r,3).font = { name: UT.font, color: { argb: UT.ink } });

  ws.getCell(10, 2).value = 'Contents';
  ws.getCell(10, 2).font = { name: UT.font, size: 12, bold: true, color: { argb: UT.headerBg } };
  let rr = 11;
  tabs.forEach((tab, i) => {
