-- 149_fac_accumulation_treaty_status_filter.sql
-- Give the TREATY leg of mv_fac_accumulation a status predicate (F35).
--
-- The facultative leg counts only r.status = 'BOUND' risks, but the treaty
-- leg had NO status filter at all, so DRAFT, DECLINED, NTU and CANCELLED
-- contracts counted as committed zone capacity in the bind-time accumulation
-- gate and in every fac_zone_budget comparison — a declined regional treaty
-- kept "occupying" its CRESTA zones forever.
--
-- The filter mirrors what "in force / live book" means everywhere else:
--   • routes/dashboard.js + facClassAccumulationService.js:
--       uw_status NOT IN ('DRAFT','DECLINED','NTU')
--     (DRAFT is not yet a position; DECLINED/NTU never went on risk)
--   • CANCELLED exists only on the contract_status mirror column (the
--     uw_workflow_status enum has no such value), so it is excluded via
--     ct.status — matching routes/retro.js and the fac dashboard's own
--     NOT IN (...,'CANCELLED') filters.
--
-- The view is otherwise copied verbatim from migration 145 (already applied
-- everywhere, so it cannot be edited), and the unique index REFRESH ...
-- CONCURRENTLY needs is recreated with the same key.
...
CREATE MATERIALIZED VIEW public.mv_fac_accumulation AS
  ...
  WHERE d.zone_id IS NOT NULL AND d.zone_id <> ''
    AND ct.uw_status::text NOT IN ('DRAFT','DECLINED','NTU')
    AND COALESCE(ct.status::text, '') <> 'CANCELLED'
  GROUP BY 1, 2, 3, 4;

-- REFRESH MATERIALIZED VIEW CONCURRENTLY (facAccumulationService.js) needs
-- this unique index — same key as migrations 137/145 created.
