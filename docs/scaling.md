...
| Dashboard portfolio-overview (`routes/dashboard.js`) | Replaced the 7 parallel queries that each re-declared the `units` CTE with 2 parallel queries that inline it as `WITH units AS MATERIALIZED (...)` and fan out into json sub-aggregates | The heavy prop+NP union now builds **2×** per tab load instead of **7×**, on **2** pooled connections instead of 7 — directly easing the pool ceiling and DB CPU. Aggregates are byte-identical (same SQL, WHEREs, GROUP BYs; order pushed into `json_agg`) | New `dashboardPortfolioOverview.integration` test pins KPIs/region/LOB/month/pivots and passes pre- and post-refactor; full `test:server` (1203) green; `EXPLAIN` confirms `units` built once, shared by N CTE Scans. p95 before/after still TODO at staging |

**Candidates — verify with the gate above before landing**

| Rank | Area | Candidate | Guardrail |
| --- | --- | --- | --- |
| 1 | Other dashboard tabs (`routes/dashboard.js`) | Apply the same `units AS MATERIALIZED` fan-out to portfolio-summary / technical-analysis tabs, which still re-declare the CTE per query. Minor: single-pass the 3× `buildPivot` JS scans over each pivot row set | Same pattern as the applied portfolio-overview change; pin each tab with a behaviour test before refactoring |
